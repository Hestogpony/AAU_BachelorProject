...
def _all_simple_paths_graph(G, source, target, cutoff=None):
    ...
    while stack:
        ...
        if child is None:
            stack.pop()
            visited.pop()
        elif path_time(G, visited) < cutoff:    #modified with path_time, from brutesolver and optimal path from the LP
            if child == target:
                yield visited + [target]
                if path_time(G, visited) < cutoff:
                    cutoff = path_time(G, visited)
                    print path_time(G, visited)
...
def path_time(G, P):
    ...
    pathtime = sum([(G[P[x]][P[(x+1)]]['weight'])*1.0 for x in xrange(len(P)-1)])
    #print pathtime
    #time.sleep(0.1)
    return pathtime
