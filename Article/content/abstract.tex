...
limited recharging possibilities as well as long recharging time of electric vehicles, the requirement for a different route planning system emerges.

We propose a greedy heuristic route planning algorithm that compute time-optimised paths, in a road network,
...
We extract map data from OpenStreetMap to create a road network for experimentation. The objective of the experiments is to analyse the route plan and the time complexity of the greedy heuristic algorithm through various experiments. This is done to asses the importance of parameters such as the charge rate of charging stations and driving distance. 
