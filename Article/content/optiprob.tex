...
The objective of the optimisation problem is to minimise the time spent driving and the time spent charging on a path from $s$ to $t$. The total time spend driving from $s$ to $t$ can be expressed as follows:
...
\todo[inline]{Beskriv hvorfor er to næsten identiske constraints nødvendigt?}
\begin{equation*}
...
The following constraints ensures that the EV is not overcharged at any charging station. It is almost exactly like the previous constraint, however this one looks one charing station further, to ensure the energy is balanced both before and after passing an edge:
