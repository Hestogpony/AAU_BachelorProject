...
$B_{cur}$ is the battery level of the EV at the start of the path, $ \sum_{j=1}^{i} EA(u_j)$ is the amount of energy acquired on the $j^{\text{th}}$ vertex including all vertices prior to vertex $i$. $\sum_{j=1}^{i} EA(u_j)$ is the energy spend on edge $j$ and as well as all edges prior to the $j^{\text{th}}$ edge.
